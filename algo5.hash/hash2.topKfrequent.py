# 692(배열이 문자열로 되어있고 리턴하는 배열값 소팅필요) 347(배열이 정수, 소팅 없음)


#문제: 주어진 list 에서 그 갯수가 가장 많은 k개의 element를 구하여라.
# 347
...
def topKfrequents(nums: List[int], k: int) -> List[int]:
  ...
  heap = []
  for num, count in hashTable.items():    # 힙큐는 원소가 들어가서 오름차순으로 정렬됨
    heapq.heappush(heap, (count, num))
    
  while k < len(heap):
      heapq.heappop(heap)     # k = len(heap)일때 멈춤, 즉 최소값이 팝되니까 남은 거는 k개만 남음
