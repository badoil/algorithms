...
# 즉 하위배열에서 바텀-없 방식으로 상위 배열이 palindromic한지 체크하면서 진행할 수 있음
# 이를 위해서 1차원인 문자열을 2차원 디피 테이블로 만들어야함
# dpTwoDimension[n][m]은 문자열의 n번째부터 m번째 까지의 하위 문자열임
# 즉 현재 dpTwoDimension 이차원 배열의 위치는 문자열의 하위문자열을 나타냄
...
# 이때 하위배열이 palindromic하면 체크하고자 하는 그 상위 배열의 시작문자와 끝문자가 동일한지만 체크하면 됨
...
def longestPalind(string: str) -> str:
  ...
  for idx in range(stringLength-1):     # 두번째 초기화작업은 문자열 2개짜리가 palindromic한지 체크
    startChar = string[idx]
    endChar = string[idx+1]
    if startChar == endChar:
      dpTwoDimension[idx][idx+1] = 2
    else:
      dpTwoDimension[idx][idx+1] = 0

  for idx in range(2, stringLength):      
    row = 0                           # 한 row에 대해서 초기화한 값 이후의 컬럼(2부터)에 대해 체크
    col = idx
    while stringLength > col:
      startChar = string[row]
      endChar = string[col]
      lastSubStringLength = dpTwoDimension[row+1][col-1]    # 2차원 배열의 현재 체크하고자하는 문자열의 위치의 바로 왼쪽 대각선이 현재 문자열의 부분문자열임
