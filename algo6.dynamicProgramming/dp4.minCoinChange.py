...
def minCoinChange(coins: List[int], sum: int) -> int:
  MIN = 999999999
  minCoinCounts = [-1] * (sum+1)    # 이 배열에 idx원을 만들기 위한 최소한의 동전 갯수가 들어감, 주어진 동전들로 sum을 못 구하면 -1 리턴, 
  minCoinCounts[0] = 0                  # sum원을 구해야 하기 때문에 마지막 인덱스가 sum이 되는 배열 필요 고로 길이가 sum+1인 배열 필요

  for idx in range(sum+1):
    if minCoinCounts[idx] != -1:
      continue

    crtMinCount = MIN
    for coin in coins:          # 반복문은 F(n) = min(F(n-coin1), F(n-coin2), F(n-coin3)) + 1 중에서 min(F(n-coin1), F(n-coin2), F(n-coin3)) 값을 구하는 과정
      ...
      lastCoinCount = minCoinCounts[lastIdx]
      crtMinCount = min(crtMinCount, lastCoinCount) + 1
